...
def extract_metrics(result):
    """从验证结果中提取 mAP50, mAP50-95, 召回率, F1 分数 兼容不同版本的 Ultralytics YOLO.
    """
    # 默认值
    ...
    if (map50 is None or map50_95 is None or recall is None or precision is None or f1 is None) and hasattr(
        result, "results"
    ):
    ...
    return {"map50": map50, "map50_95": map50_95, "recall": recall, "f1": f1}
    ...
    if successful_trains:
        ...
        if val_results:
            ...
            for seed, result in val_results.items():
                ...
                print(
                    f"{seed:<8} {metrics['map50']:<10.4f} {metrics['map50_95']:<12.4f} "
            ...
            if metrics_list:
                ...
                print("\n📈 统计信息:")
                print(
                    f"mAP@50   - 平均值: {np.mean(map50_vals):.4f} 标准差: {np.std(map50_vals):.4f}  "
                ...
                with open(stats_file, "w", encoding="utf-8") as f:
                    ...
                    for i, seed in enumerate(val_results.keys()):
                        m = metrics_list[i]
                        f.write(
                            f"种子 {seed}: mAP@50={m['map50']:.4f}, mAP@50-95={m['map50_95']:.4f}, "
