...
def extract_class_metrics(val_results):
    ...
    class_names = val_results.names if hasattr(val_results, 'names') else {i: str(i) for i in range(len(val_results.box.ap_class_index))}
...
def plot_average_loss_accuracy_curves(seeds, project_dir, save_path):
    ...
    for m in ['train/box_loss', 'train/cls_loss', 'train/dfl_loss']:
        ...
        plt.fill_between(epochs, np.array(avg_data[m])-np.array(std_data[m]), np.array(avg_data[m])+np.array(std_data[m]), alpha=0.2)
    plt.xlabel('Epoch'); plt.ylabel('Loss'); plt.title('Training Loss (Avg ± Std)'); plt.legend(); plt.grid(True)
    ...
    for m in ['val/box_loss', 'val/cls_loss', 'val/dfl_loss']:
        plt.plot(epochs, avg_data[m], label=m.split('/')[-1].replace('_loss', ''))
        plt.fill_between(epochs, np.array(avg_data[m])-np.array(std_data[m]), np.array(avg_data[m])+np.array(std_data[m]), alpha=0.2)
    plt.xlabel('Epoch'); plt.ylabel('Loss'); plt.title('Validation Loss (Avg ± Std)'); plt.legend(); plt.grid(True)

    # 精确率
    plt.subplot(2, 4, 3)
    m = 'metrics/precision(B)'
    plt.plot(epochs, avg_data[m], 'b-', label='Precision')
    plt.fill_between(epochs, np.array(avg_data[m])-np.array(std_data[m]), np.array(avg_data[m])+np.array(std_data[m]), alpha=0.2, color='b')
    plt.xlabel('Epoch'); plt.ylabel('Precision'); plt.title('Precision (Avg ± Std)'); plt.grid(True)

    # 召回率
    plt.subplot(2, 4, 4)
    m = 'metrics/recall(B)'
    plt.plot(epochs, avg_data[m], 'g-', label='Recall')
    plt.fill_between(epochs, np.array(avg_data[m])-np.array(std_data[m]), np.array(avg_data[m])+np.array(std_data[m]), alpha=0.2, color='g')
    plt.xlabel('Epoch'); plt.ylabel('Recall'); plt.title('Recall (Avg ± Std)'); plt.grid(True)

    # mAP50
    plt.subplot(2, 4, 5)
    m = 'metrics/mAP50(B)'
    plt.plot(epochs, avg_data[m], 'r-', label='mAP50')
    plt.fill_between(epochs, np.array(avg_data[m])-np.array(std_data[m]), np.array(avg_data[m])+np.array(std_data[m]), alpha=0.2, color='r')
    plt.xlabel('Epoch'); plt.ylabel('mAP50'); plt.title('mAP@50 (Avg ± Std)'); plt.grid(True)

    # mAP50-95
    plt.subplot(2, 4, 6)
    m = 'metrics/mAP50-95(B)'
    plt.plot(epochs, avg_data[m], 'm-', label='mAP50-95')
    plt.fill_between(epochs, np.array(avg_data[m])-np.array(std_data[m]), np.array(avg_data[m])+np.array(std_data[m]), alpha=0.2, color='m')
    plt.xlabel('Epoch'); plt.ylabel('mAP50-95'); plt.title('mAP@50-95 (Avg ± Std)'); plt.grid(True)
...
def main():
    ...
    for cls in all_classes:
        ...
        if map50:
            rows.append({
                'Class': cls,
                'mAP50_mean': np.mean(map50), 'mAP50_std': np.std(map50),
                'mAP50-95_mean': np.mean(map95), 'mAP50-95_std': np.std(map95),
